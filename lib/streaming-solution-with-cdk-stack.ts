...
export class StreamingSolutionWithCdkStack extends cdk.Stack {
  constructor(scope: cdk.Construct, id: string, props?: cdk.StackProps) {
    ...
    const abnormalNotificationTopic = new sns.Topic(
      this,
    ...
    abnormalNotificationTopic.addSubscription(
      new snssub.EmailSubscription('18it001@charusat.edu.in'),
    )
    ...
    streamToAnalyticsRole.addToPolicy(
      new iam.PolicyStatement({
        resources: [fanoutLambda.functionArn, rootStream.streamArn],
        actions: ['kinesis:*', 'lambda:*'],
      }),
    )

    const thresholdDetector = new kinesisanalytics.CfnApplication(
      this,
      'KinesisAnalyticsApplication',
      {
        applicationName: 'abnormality-detector',
        applicationCode: fs
          .readFileSync(path.join(__dirname, 'src/app.sql'))
          .toString(),
        inputs: [
          {
    ...
    const thresholdDetectorOutput = new kinesisanalytics.CfnApplicationOutput(
      this,
      'AnalyticsAppOutput',
      {
        applicationName: 'abnormality-detector',
